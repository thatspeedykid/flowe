...
class _BudgetScreenState extends State<BudgetScreen> {
  ...
  void _exportCSV() {
    final b = _budget;
    final rows = ['Label,Amount,Type'];
    for (final s in b.income)  for (final r in s.rows) rows.add('${r.label},${r.amount.toStringAsFixed(2)},Income');
    for (final s in b.expense) for (final r in s.rows) rows.add('${r.label},${r.amount.toStringAsFixed(2)},Expense');
    ...
    final path = '${Platform.environment['HOME']}/flo_budget_$_key.csv';
    File(path).writeAsStringSync(rows.join('\n'));
    _showToast('Saved ~/flo_budget_$_key.csv');
  }

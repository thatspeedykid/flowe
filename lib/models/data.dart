  ...
  static Future<File> _getFile() async {
    ...
    if (!await file.exists()) {
      ...
      if (Platform.isAndroid) {
        ...
        // Desktop: cover all known previous data locations
        // path_provider on Windows returns %APPDATA%\<appName>\<appName>\
        // path_provider on Linux returns ~/.local/share/<appName>/<appName>/
        // App has been named: flo, flowe — cover all combinations
        final appdata = Platform.environment['APPDATA'] ?? '';
        final home = Platform.environment['HOME'] ?? '';
        legacyPaths.addAll([
          // Windows — flowe variants (previous flutter app name)
          '$appdata\\flowe\\flowe\\data.json',
          '$appdata\\flowe\\data.json',
          // Windows — flo variants (v1.x HTML app)
          '$appdata\\flo\\flo\\data.json',
          '$appdata\\flo\\data.json',
          // Linux — flowe variants
          '$home/.local/share/flowe/flowe/data.json',
          '$home/.local/share/flowe/data.json',
          // Linux — flo variants (v1.x)
          '$home/.local/share/flo/flo/data.json',
          '$home/.local/share/flo/data.json',
        ]);
      }
